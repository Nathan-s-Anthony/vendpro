        ...
        <>
            {
                toggledModal ?
                    <div onClick={() => (setToggleModal(!toggledModal))} className={`${toggled ? "modal-container-full" : "modal-container-compact"}   flex items-center justify-center p-6 ml-auto fixed   inset-0 z-50 w-screen h-screen`}>
                        <div className="test bg-secondary p-10 max-w-180 w-full rounded-sm lg:h-[70vh] relative  border-border/30 ">
                            <X className=" cursor-pointer justify-self-end" onClick={() => setToggleModal(!toggledModal)} />
                            ...
                            <div className=" flex items-center justify-center">
                                <div className="grid grid-cols-3  gap-5 mb-4 mt-4">
                                    {data.map((machine) => {
                                        console.log(machine.image.data, "machine logging")
                                        return (
                                            <Tile
                                                key={machine.id}
